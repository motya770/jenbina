...
def _load_or_create_person(user_db: UserDatabase, user_id: int) -> Person:
    ...
    # Ensure LLM-dependent systems are initialised
...
def init_session_state():
    ...
    # Person — load from DB if authenticated
...
def main():
    ...
    if not render_auth_page():
        return  # Not logged in yet — show login UI only

    user = st.session_state.current_user
    render_user_header(user)

    # ── Initialise session (loads user-specific Person state) ─────────────
    init_session_state()
